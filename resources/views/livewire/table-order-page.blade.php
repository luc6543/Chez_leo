    ...
    <div class="w-full h-full flex flex-col gap-10 items-start justify-start">
        ...
        @foreach($this->products as $category => $products)
                <div x-data="{openMenu : false}" class="w-full">
                    ...
                    <div class="flex shadow rounded bg-white flex-wrap gap-5 justify-around w-full h-fit" x-show="openMenu" x-cloak>
        ...
        <div class="bottom-0 fixed left-0 w-screen flex justify-around">
            ...
            <button wire:click.prevent="billPaid" class="w-1/3 shadow px-4 text-white bg-emerald-800 p-2 rounded">Rekening betaald <span>Totaal ${{ $reservation->bill->getSum() }}</span></button>
        </div>
    </div>
</div>
